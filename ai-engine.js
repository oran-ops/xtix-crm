...
'use strict';
  ...
  function _buildAnalysisPrompt(lead, METH, compInfo) {
    ...
    var SCORING_GUIDE = [
      '=== מדריך ציון 0-100 ===',
      'ציון גבוה 70-100 (ליד חם): מוכר כרטיסים כבר + כאב ברור (אתר חיצוני/מכירה ידנית/אין data) + פעילות קבועה + גודל משמעותי.',
      'ציון בינוני 40-69 (פוטנציאל): מוכר כרטיסים אבל צורך לא דחוף / פעילות קטנה / לא ברור כאב.',
      'ציון נמוך 0-39 (לא רלוונטי): לא מוכר כרטיסים / מוסד שלא יכול לשנות / כבר עם XTIX.',
      'חשוב: תן ציון על בסיס התמונה הכוללת — כל ליד יש לו נקודת חולשה ייחודית.'
    ].join('\n');
  ...
  async function _metaJudge(lead, baseResult, gptResult, geminiResult, history, engineContext) {
    ...
    var user = 'ליד: ' + (lead.name||'') + ' | פלטפורמה: ' + (lead.platform||'') +
               ...
               '\n\nהחלט:\n1. מה הציון הסופי (0-100)? נמק.\n2. מה המלצת הפעולה?\n3. על מה המנועים חלקו?\n4. מה רמת הביטחון שלך?' +
               '\n\nהחזר JSON: { "meta_score": N, "meta_reasoning": "...", "meta_action": "...", ' +
               '"disagreements": [...], "confidence": "high/mid/low", ' +
               '"final_summary": "...", "final_tier": "A/B/C" }';
